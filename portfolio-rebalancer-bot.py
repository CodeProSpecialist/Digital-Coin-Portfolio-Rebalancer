...
def start_market_websocket():
    ...
    chunks = [all_streams[i:i + MAX_STREAMS_PER_CONNECTION]
...
def sell_to_usdt(bot: RebalancerBot, sym: str, qty: Decimal):
    ...
    limit_price = (best_bid // tick) * tick
...
def rebalance_portfolio(bot: RebalancerBot):
    ...
    for b in acct["balances"]:
        ...
        if qty > ZERO and not is_stablecoin(asset):
            sym = f"{asset}USDT"
            if sym in valid_symbols:
                positions[sym] = {"asset": asset, "qty": qty}

    target_val = total_value * PERCENTAGE_PER_COIN
    ...
    for sym, info in positions.items():
        ...
        if value > target_val:
            excess_qty = (value - target_val) / price
            step = bot.get_lot_step(sym)
            excess_qty = excess_qty.quantize(step, rounding=ROUND_DOWN)

            if excess_qty > ZERO:
                logger.info(f"EXCESS {sym}: {excess_qty} (${value - target_val:.2f})")
...
def main():
    ...
    logger.info("Waiting 30s for WebSocket synchronization...")
    time.sleep(30)
